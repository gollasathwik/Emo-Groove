...
'''
    ...
    This function readf the content in a directory and for each mp3 file detected
    readf the file and extracts relevant features using librosa library for audio
    signal processing
'''
...
def extract_feature(path):
    ...
    songname_vector = pd.Series(dtype='string')
    ...
    rms_mean = pd.Series(dtype='float64')
    rms_std = pd.Series(dtype='float64')
    rms_var = pd.Series(dtype='float64')
    ...
    for line in file_data:
        ...
        # Reading Song
        songname = path + "\\" + line
        y, sr = librosa.load(songname, duration=135)
    ...
    feature_set.to_csv('Emotion_features_extract_long.csv')
    feature_set.to_json('Emotion_features_extract_long.json')


# Extracting Feature Function Call
extract_feature("D:\Semester\songs")
